...
import { useBeans, useSettings } from '../ui/data';
...
/**
 * レシピの作成・編集の全画面シート。
 * `editing` があれば編集、なければ設定の初期値から作成する。
 */
export function RecipeWizard({ editing, onClose }: { editing?: Recipe; onClose: () => void }) {
  const beans = useBeans();
  const defaults = useSettings().recipeDefaults;
  const [draft, setDraft] = useState<Draft>(() => (editing ? draftOf(editing) : emptyDraft(defaults)));
      ...
      >
        ...
        <div className="wizard-foot">
          ...
          {step === STEPS.length - 1 ? (
            ...
            <button className="primary" type="button" disabled={nextDisabled} onClick={() => setStep((current) => current + 1)}>
              次へ
            </button>
          )}
        </div>
